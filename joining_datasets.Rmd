...
Hydrologists often need to join time series which do not have values at the same time intervals. Fortunately, the `dplyr` package has functions which can help. The following is a simple example of how you can align time series.
...
We can now align the data sets, using the `inner_join` function from the package **dplyr**. This function only selects peaks which have mathcing key values, in this case the years. Because the `Date` variable was in both data frames, both are in the joined data frame.
